...
const handler = async (m, { conn }) => {
    ...
    await conn.sendMessage(m.chat, {
        ...
        caption:
            ...
            `╰─┈─┈─┈─⟞🏦⟝─┈─┈─┈─╯\n\n` +
            `┃ @${m.sender.split('@')[0]}\n` +
            `┃ 🪙 الذهب: ${formatNumber(user.coins)}\n` +
            `┃ 💎 الألماس: ${formatNumber(user.diamonds)}\n` +
            `┃ 💵 الدولار: ${formatNumber(user.dollars)}\n` +
            `┃ 🎖️ رتبتك: ${user.nameLevel || '🎪 مشاهد'}\n` +
            `┃ ⭐ مستواك: ${user.level || 0}\n` +
            `┃ 💬 الرسائل: ${user.msgcount || 0}`,
        contextInfo: { mentionedJid: [m.sender] }
    }, { quoted: global.reply_status || m });
};

handler.usage    = ['بنك', 'bank', 'رصيدي'];
handler.category = 'bank';
handler.command  = ['بنك', 'bank', 'رصيدي'];
